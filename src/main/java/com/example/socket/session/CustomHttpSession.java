...
public interface CustomHttpSession {
    ...
    void setMaxInactiveInterval(int interval);  //세션을 유지할 시간을 초단위로 설정 합니다.

    int getMaxInactiveInterval(); // 세션의 유효시간을 초 단위로 반환 합니다. 기본값은 30초 입니다.

    Object getAttribute(String name); //get attribute

    void setAttribute(String name, Object value); //attribute 설정

    void removeAttribute(String name); //attribte 삭제

    void invalidate(); //현재 세션을 종료. 세션관련 모든 값 삭제

    boolean isNew(); //session이 만들어졌는지 boolean 타입으로 반환 합니다.
    Map<String, ThreadLocal<String>> getSessionStore();
